import { DeleteTextsBody, ProcessMessage } from 'api/controllers/messages.controllers';
...
export class MessagesService {
  ...
  async processMessages({ messages, host, protocol }: { messages: ProcessMessage, protocol: string, host: string }) {
        ...
        const processingPromises = files.map(async (file) => {
          try {
            ...
            switch (file.fileType) {
              case 'txt':
                textContent = await this.processTxt(file);
                break;
              case 'pdf':
  ...
  async deleteTexts({ conversationId, filename }: DeleteTextsBody): Promise<{
    error: boolean;
    status: number;
    message: string;
    deletedFiles: string[];
    ...
    try {
      ...
      const allFiles = fs.readdirSync(TEXTS_DIR);
  ...
  private async processTxt(file: FileInput): Promise<string> {
  ...
  private async processImage(file: FileInput): Promise<string> {
    const { fileId, url } = file;
  ...
  private async processPdf(file: FileInput): Promise<string> {
    const { fileId, url } = file;
    ...
    try {
      ...
      const processChunk = (chunkPages: string[]) => {
        return new Promise<string[]>((resolve, reject) => {
          // Para produção, usa .js; para desenvolvimento, pode precisar do caminho do arquivo compilado
  ...
  private async processDocx(file: FileInput): Promise<string> {
    const { fileId, url } = file;
