...
export class SectionDetectorService {
  ...
  private readonly SECTION_PATTERNS = [
    ...
    {
      regex: /^([A-ZÀÂÃÁÉÊÍÓÔÕÚÇ]{2,}(?:\s+[A-ZÀÂÃÁÉÊÍÓÔÕÚÇ]{2,}){2,})\s*$/m,
  ...
  detectSections(text: string, pageBreaks: PageBreak[]): DocumentSection[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!hasLeadingWhitespace) {
        ...
        if (match) {
          const pageNumber = this.findPageForCharIndex(currentCharIndex, pageBreaks);
    ...
    this.calculateSectionEndPages(sections, pageBreaks[pageBreaks.length - 1]?.pageNumber ?? 1);
   ...
   */
  private matchesSectionPattern(line: string): { level: number } | null {
    // Length constraints to avoid false positives
    if (line.length < this.MIN_LINE_LENGTH || line.length > this.MAX_LINE_LENGTH) {
      return null;
    }
   ...
   */
  private findPageForCharIndex(charIndex: number, pageBreaks: PageBreak[]): number {
   ...
   */
  private calculateSectionEndPages(sections: DocumentSection[], lastPage: number): void {
